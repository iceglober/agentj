...
- `core/lib/agent/` — agent assembly. `index.ts` composes llm + prompt + tools; toolsets follow `mode` ("plan" = read/search (+ research subagents), "build" = bash/search/edit (+ worktree delegation)) crossed with `role` (primary/delegate). `permissions.ts`: ask/allow/deny policy + `withPermissions` tool wrapper (mutating tools only, opt-in). `subagents.ts`: the unified `run_subagents` task-DAG tool over `scheduler.ts` (bounded dependency scheduler); capability injected per mode. `agent.tools.subagents.model` tier-routes children.
