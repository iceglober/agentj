...
- `core/`
  ...
  - Key areas:
    - `core/agent-loop.ts` — the sole production composition root. Selects and connects config, secret, metrics, sandbox, session, agent, CLI, and TUI adapters, then delegates behavior to injectable application services. Owns SIGINT handler lifecycle.
    - `core/lib/app/` — orchestration services. `conversation.ts` emits safe sandbox image/bootstrap-count lifecycle events before setup, then owns the in-process plan → feedback → explicit approval → build lifecycle while retaining one sandbox/session; `build-report.ts` gates shipped commits on a structured completion report, observed passing validation, and no failed tools. Blocked changed builds become recovery commits. `run.ts` retains the injectable one-shot task primitive. None selects production adapters.
    ...
    - `core/lib/tui/` — terminal interaction and rendering. `index.ts` defines injectable `PromptUi` for initial tasks and follow-up feedback/approval, `TerminalWriters`, and a line-oriented `TranscriptRenderer` with Plan, Feedback, Build, Result, and Commit sections. Tool payloads are safely truncated via `safeRenderJson`. No fullscreen state machine.
    - `core/lib/agent/` — purpose-specific agent assembly and delegation. `scheduler.ts` is the shared bounded dependency scheduler; builders add workspace snapshots/integration while planners add read-only lane normalization and progress events. Planning workers cannot delegate. `agent.tools.subagents.concurrency` bounds both implementations.
    - `core/lib/session/` — worktree lifecycle. `session.base: auto` compares fetched remote-default and matching local branches by ancestry, choosing the descendant or the remote baseline on divergence. Creates branches/worktrees from the selected ref; child lanes never force-delete uncertain work.
    - `core/lib/session-store/` — durable AgentJ task/plan/feedback/phase manifests used by `--resume`; the file adapter writes atomically under the user state directory.
    - `core/lib/workspace/` — execution and source-state infrastructure. Host execution runs ordinary `aj` directly in the caller's checkout; Git delegation snapshots include dirty parent state and integrate child commits back without touching the user's index.
...
- `core/agent-loop.ts` is the only composition root: it selects every concrete production adapter and injects them into the CLI and application services. Domain modules never construct production dependencies.
- Ordinary task routes select the host execution environment and a non-owning local session; `sandbox` routes select an adapter from the sandbox provider registry and retain isolated worktree behavior.
- `core/lib/app/conversation.ts` owns phase transitions and resource lifetime for one in-process conversation. It calls stateless agent generations with bounded relevant context; `core/lib/app/run.ts` retains the one-shot primitive for direct internal/eval use.
...
- The TUI is line-oriented and multi-turn within one process. Its only cursor-addressed region is a fixed-height planning-subagent ledger with spinner and elapsed-time rows; non-TTY output uses append-only lifecycle lines. There is no fullscreen state machine or cross-invocation persistence.
