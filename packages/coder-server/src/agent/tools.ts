...
import { availableTasks, detectProjectFacts, loadFactsFile, persistFacts, type ProjectPattern, refreshProjectFacts, resolveCommand, toolchainForPath } from "../project/facts.ts";
...
export const COMMAND_TOOLS: ReadonlySet<string> = new Set(["script", "bash"]);
...
export function makeTools(deps: ToolDeps): ToolSet {
  ...
  return {
    ...
    script: tool({
      ...
      execute: async ({ task, path, args }) => {
        try {
          const facts = await detectProjectFacts(root);
          const resolved = resolveCommand(facts, task, path, args);
          if (!resolved) {
