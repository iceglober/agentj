...
// merged OVER a agentj-global `~/.agentj/.mcp.json` (repo entries win on a name clash). We deliberately
...
export function mcpConfigPaths(root: string): { global: string; repo: string } {
  return { global: join(homedir(), ".agentj", ".mcp.json"), repo: join(root, ".mcp.json") };
}
