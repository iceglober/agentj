...
// one-time interactive flow (used by `agentj mcp login`); routine runs never reach this — they load
...
const OAUTH_PORT = Number(process.env.AGENTJ_OAUTH_PORT) || 41100;
...
/**
 * An OAuthClientProvider for one server, persisting to `~/.agentj/auth.json`. `interactive` controls
 * `redirectToAuthorization`: during `agentj mcp login` it opens the browser; during a normal run it
 ...
 */
export class AgentjOAuthProvider implements OAuthClientProvider {
  constructor(
    private readonly server: string,
    private readonly interactive: boolean,
    /** Notified with the authorization URL when redirecting. Defaults to stderr (CLI). */
  ...
  async codeVerifier(): Promise<string> {
    ...
    if (!v) throw new Error("missing PKCE code verifier — re-run `agentj mcp login`");
    return v;
  }
  ...
  redirectToAuthorization(authorizationUrl: URL): void {
    ...
    else process.stderr.write(`[agentj] opening your browser to authorize…\n  if it doesn't open, visit:\n  ${url}\n`);
    openBrowser(url);
  }
}
...
export async function loginToServer(server: string, url: string, opts: { onRedirect?: (url: string) => void } = {}): Promise<void> {
  const provider = new AgentjOAuthProvider(server, true, opts.onRedirect);
